...
//    def union(that:TweetSet): TweetSet = {
//      if(that.contains(elem)) union(that.remove(elem))
...
//    }
  ...
  def descendingByRetweet: TweetList= new Cons (mostRetweeted, left.union(right).descendingByRetweet)

  def mostRetweeted: Tweet = {
    def aux(tw1: Tweet, tw2: Tweet): Tweet ={
      if(tw1.retweets > tw2.retweets) tw1
      else tw2
    }
    if (left.isEmpty && right.isEmpty) elem
    else if(left.isEmpty) aux(right.mostRetweeted, elem)
    else if(right.isEmpty) aux(left.mostRetweeted, elem)
    else aux(left.mostRetweeted,right.mostRetweeted)
  }
